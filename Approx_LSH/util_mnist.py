...
class TestGroup(object):
    ...
    def run(self, k=None, epoch=None):
        ...
        model = MLP(self.hidden, self.layer, self.inputs, self.output)
        model.reset_parameters()

        # model.cpu()

        opt = optim.Adam(model.parameters(),eps=1e-4)
        #opt  = optim.Adagrad(model.parameters(), lr=0.01, eps=1e-4)
        acc = 0  # best dev. acc.
        ...
        for t in range(epoch):
            ...
            print("(feedfoward time: {:.3f} sec, backpropagation time: {:.3f} sec) ".format(ft, bt), end='')
            losses.append(loss)
            ftime.append(ft)
            btime.append(bt)
            utime.append(ut)
        ...
        file_object.close()
        torch.save(model, "trained_model"+str(model.layer))
