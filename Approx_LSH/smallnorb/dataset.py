...
class SmallNORBDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_root, length,training = True, offset=0):
        """
        ...
        """
        self.offset = offset  
        self.length = length  # length of test data
        ...
        self.dataset[self.datasplit]= self._fill_data_structures(self.datasplit)
        #self.dataset['dev']= self.dataset['train'][:self.offset+self.length]
        #print(self.dataset[self.data_split]) 
            #self.dataset[data_split] = torch.Tensor(self.dataset[data_split])

        self.initialized = True
        super(SmallNORBDataset, self).__init__()
    
    def __len__(self):
        return len(self.dataset['train']['clst'])+len(self.dataset['test']['dat'])
    
    def __getitem__(self,idx):
     
        #self.dataset['dev']= self.dataset['train'][:self.offset+idx]
        #self.dataset['train']= self.dataset['train'][self.offset+idx:]
        #self.dataset['test']=self.dataset['test'][:self.length]
        return self.dataset[self.datasplit]['dat'][idx], self.dataset[self.datasplit]['clst'][idx]
    ...
    def export_to_jpg(self, export_dir, split_name):
        """
    ...
    def group_dataset_by_category_and_instance(self, dataset_split):
        ...
        groups = []
        for key, group in groupby(iterable=sorted(self.dataset[dataset_split]),
    ...
    def _fill_data_structures(self, dataset_split):
        ...
        for i in range(SmallNORBDataset.n_examples):
         #   small_norb_example.image_lt   = dat_data[2 * i]
         #   small_norb_example.image_rt   = dat_data[2 * i + 1]
         #   small_norb_example.category  = cat_data[i]
            #print(type(dat_data[i]))
            #image = torch.from_numpy(dat_data[i]).float()
            cat = torch.tensor(cat_data[i]).float()
            #x = torchvision.transforms.ToPILImage()(image)
            #x = torchvision.transforms.Grayscale()(x)
            #x = torchvision.transforms.ToTensor()(x)
            #image = x[0, :, :]
            
            #self.dataset[dataset_split]['dat'].append(image)
            #self.dataset[dataset_split]['clst'].append(cat)
            image = torch.from_numpy(dat_data[2 * i].astype(np.float64))
            image2 = torch.from_numpy(dat_data[2 * i+1].astype(np.float64))
            transform = transforms.Compose([torchvision.transforms.Normalize(mean = (0.2502,),
                         std= (0.1755,))])
            #cat = transform(cat_data[i].astype(np.float64))
            #image = transform(.astype(np.float64))
            x = torchvision.transforms.ToPILImage()(image)
            x1 = torchvision.transforms.ToPILImage()(image2)
            x = torchvision.transforms.Grayscale()(x)
            x1 = torchvision.transforms.Grayscale()(x1)

            x = transform(torchvision.transforms.ToTensor()(x))
            x1 = transform(torchvision.transforms.ToTensor()(x1))
            transform = transforms.Compose([torchvision.transforms.Normalize(mean = (0.2502,),
                         std= (0.1755,))])
            image = x[0, :, :]
            image2 = x1[0, :, :]
            #self.dataset[dataset_split]['dat'].append(image)
            self.dataset[dataset_split]['dat'].append(image)
            self.dataset[dataset_split]['dat'].append(image2)
            #self.dataset[dataset_split]['dat'].append(image2)
            self.dataset[dataset_split]['clst'].append(cat)
            self.dataset[dataset_split]['clst'].append(cat)
         ...
         #   small_norb_example.azimuth   = info_data[i][2]
        #  small_norb_example.lighting  = info_data[i][3]
        return self.dataset[dataset_split]
    ...
    def _parse_NORB_dat_file(file_path):
        ...
        with open(file_path, mode='rb') as f:
            ...
            dtype = np.dtype('float')
            examples = np.zeros(shape=(num_examples * channels, height, width), dtype=dtype)
    ...
    def _parse_NORB_info_file(file_path):
        ...
        with open(file_path, mode='rb') as f:
            ...
            examples = np.zeros(shape=(num_examples, num_info), dtype=np.float)
